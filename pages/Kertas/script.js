...
const dr=document.getElementById('draw'),drx=dr.getContext('2d')
const overlay=document.getElementById('overlay'),ox=overlay.getContext('2d')
...
function setSize(w,h){stack.style.width=w+'px';stack.style.height=h+'px'}
function apply(){stack.style.transform=`translate(-50%,-50%) translate(${px}px,${py}px) scale(${sc})`}
...
function init(){drx.clearRect(0,0,dr.width,dr.height);frames.push(dr.toDataURL());cur=frames.length-1;render()}
...
document.addEventListener('mousedown',e=>{if(!pan)return;psx=e.clientX-targetPx;psy=e.clientY-targetPy;document.body.style.cursor='grab'})
...
brushBtn.onclick=()=>{eras=!eras;brushBtn.textContent=eras?'🧽':'✏️'}
...
//document.getElementById('clr').onclick=()=>{drx.clearRect(0,0,dr.width,dr.height);frames[cur]=dr.toDataURL();render()}
...
document.getElementById('save').onclick=()=>{const tmp=document.createElement('canvas');tmp.width=bg.width;tmp.height=bg.height;const tx=tmp.getContext('2d');tx.drawImage(bg,0,0);tx.drawImage(dr,0,0);const link=document.createElement('a');link.download='img.png';link.href=tmp.toDataURL();link.click()}

function add(){drx.clearRect(0,0,dr.width,dr.height);frames.push(dr.toDataURL());cur=frames.length-1;render()}
function render(){
...
function show(i){if(frames[i]){const img=new Image();img.onload=()=>{drx.clearRect(0,0,dr.width,dr.height);drx.drawImage(img,0,0)};img.src=frames[i]}}

document.getElementById('export').onclick=()=>{
  const bgUrl = document.getElementById('url').value.trim()
  const data = {
    background: bgUrl, // store the image URL input
    frames: frames.map((f,i)=>({index:i,url:f}))
  }
...
async function composeFrame(bgUrl, frameUrl, outW, outH){
  ...
  if(frameUrl){
    ...
    const w = frImg.width * r, h = frImg.height * r
    const x = (outW - w)/2, y = (outH - h)/2
...
// Export: all frames into one PDF, sequential and stable
...
document.getElementById('load').onclick=()=>{const u=document.getElementById('url').value.trim();if(!u)return;const img=new Image();img.crossOrigin="anonymous";img.onload=()=>{resize(img.width,img.height);bgx.clearRect(0,0,bg.width,bg.height);bgx.drawImage(img,0,0);drx.clearRect(0,0,dr.width,dr.height);frames[cur]=dr.toDataURL();render()};img.src=u}
...
dr.addEventListener('mousemove', e => {
  if (selecting && e.buttons === 1) {
    selEndX = e.offsetX; selEndY = e.offsetY;
...
document.getElementById('cut').onclick = () => {
  document.getElementById('copy').onclick();
  drx.clearRect(Math.min(selStartX, selEndX), Math.min(selStartY, selEndY),
                Math.abs(selEndX - selStartX), Math.abs(selEndY - selStartY));
  frames[cur] = dr.toDataURL();
  render();
};
