...
document.addEventListener("keydown", e => {
  ...
  if (e.key === "[" || e.key === "]") {
    ...
    if (e.key === "[") {
      if (activeTool === "ToolBrush" || activeTool === "ToolRectangleFill" || activeTool === "ToolRectangleEraser") { brush_size = Math.max(1, brush_size - 1); updateBrushSizeLabel(); }
      else if (activeTool === "ToolEraser") { eraser_size = Math.max(1, eraser_size - 1); updateEraserSizeLabel(); }
    } else {
      if (activeTool === "ToolBrush" || activeTool === "ToolRectangleFill" || activeTool === "ToolRectangleEraser" ) { brush_size += 1; updateBrushSizeLabel(); }
