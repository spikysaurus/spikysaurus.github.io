...
document.getElementById('saveAssetsBtn').onclick = () => {
  ...
  for (const label in levels) {
    ...
    levels[label].forEach(c => f.file(c.name, c.data.split(',')[1], { base64: true }));
  }

  // Save backdrop as full canvas snapshot
  if (backdropCanvas) {
    const dataURL = backdropCanvas.toDataURL("image/png");
    zip.folder("backdrop").file("backdrop.png", dataURL.split(',')[1], { base64: true });
  }
...
function fileLoadAssets(file) {
  JSZip.loadAsync(file).then(zip => {
    levels = {}; levelCount = 0; levelsTree.innerHTML = "";
    activeLevel = null; activeDrawing = null;
    ...
    if (propFile) propFile.async("string").then(content => {
    ...
    zip.folder("drawings").forEach((rel, entry) => {
      if (entry.dir) {
        ...
        div.className = 'level'; div.dataset.label = label;
        const toggle = document.createElement('span');
        toggle.className = 'toggle-icon'; toggle.innerHTML = '<span class="bl-icons-outliner"></span>';
        const lblInput = document.createElement('input');
        lblInput.type = 'text'; lblInput.value = label; lblInput.className = 'level-label';
        lblInput.onblur = () => {
          const newLabel = lblInput.value.trim();
          if (newLabel && newLabel !== label) {
            levels[newLabel] = levels[label]; delete levels[label];
            div.dataset.label = newLabel; activeLevel = newLabel;
            activeLevelLabel.textContent = `Active Level: ${newLabel}`; autoArrange(newLabel);
          }
        };
        lblInput.onclick = () => {
          document.querySelectorAll('.level-label').forEach(el => el.classList.remove('active'));
          lblInput.classList.add('active'); activeLevel = lblInput.value.trim();
        ...
        div.append(toggle, lblInput, list); levelsTree.appendChild(div); levelCount++;
      } else {
        const parts = rel.split("/"), label = parts[0], name = parts[1];
        const p = zip.file(entry.name).async("base64").then(data => {
          levels[label].push({ name, data: `data:image/png;base64,${data}` });
        });
        drawingPromises.push(p);
      }
    });

    Promise.all(drawingPromises).then(() => { for (const label in levels) autoArrange(label); });
    autoArrangeLevels();
  });
}
...
dropZoneTree.addEventListener("dragover", e => { e.preventDefault(); dropZoneTree.style.opacity = "0.5"; });
dropZoneTree.addEventListener("dragleave", () => { dropZoneTree.style.opacity = "1"; });
dropZoneTree.addEventListener("drop", e => {
  e.preventDefault(); dropZoneTree.style.opacity = "1";
  const file = e.dataTransfer.files[0]; if (file) fileLoadAssets(file);
});
...
document.getElementById('backdropInput').onchange = e => {
  ...
  reader.onload = evt => {
    ...
    const img = new Image();
    img.onload = () => {
      backdropCanvas.width = img.width; backdropCanvas.height = img.height;
      backdropCanvasCtx.clearRect(0, 0, img.width, img.height);
      backdropCanvasCtx.drawImage(img, 0, 0); canvasRefreshInputValues();
    };
    img.src = backdropData;
  };
...
assetsInput.addEventListener("change", e => { const file = e.target.files[0]; if (file) fileLoadAssets(file); });
